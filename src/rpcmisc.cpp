...
UniValue validateaddress(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"flcaddress\"\n"
            "\nReturn information about the given flc address.\n"
            "\nArguments:\n"
            "1. \"flcaddress\"     (string, required) The flc address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"flcaddress\", (string) The flc address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: FLC address and we have full public key:
...
UniValue createmultisig(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are flc addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) flc address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
UniValue verifymessage(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"flcaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"flcaddress\"  (string, required) The flc address to use for the signature.\n"
