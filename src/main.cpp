...
/** Fees smaller than this (in uflc) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
     ...
     } else if (nHeight == 1)                          { nSubsidy = 250000 * COIN; //premimed
     } else if (nHeight > 1 && nHeight <= 200)       { nSubsidy = 100 * COIN;
    } else { nSubsidy = 0.00001 * COIN; }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FLC: We use certain sporks during IBD, so check to see if they are
